...
exports.plugin = function (schema, options) {
    ...
    IdentityCounter.findOne({
        ...
        .then(counter => {
            if (!counter) {
                ...
                ready = true;
                return Promise.resolve(); // Return a resolved promise if counter exists
            }
        })
        .then(() => {
            ready = true; // Set ready to true after the save operation completes
        })
        .catch(err => {
    ...
    const resetCount = () => {
        ...
        return IdentityCounter.findOneAndUpdate(
            ...
            { count: settings.startAt - settings.incrementBy },
            { new: true } // new: true specifies that the promise should resolve with the updated counter.
        )
            .then(() => {
                    ...
                    if (typeof doc[settings.field] === 'number') {
                        ...
                        IdentityCounter.findOneAndUpdate(
                            ...
                            .then(updatedIdentityCounter => {
                                // If there are no errors, set the document's field to the current count.
